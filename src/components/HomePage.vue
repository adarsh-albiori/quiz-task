    ...
    <div v-for="(item, index) in filteredArray" :key="index">
      ...
      >
        ...
        <v-card-actions class="mb-2 mr-1">
          ...
          <v-btn
            v-if="
              (qData[index].createdBy.name == store.loggedInUser.name &&
                qData[index].createdBy.email == store.loggedInUser.email) ||
              store.isAdmin === true
            "
            color="green"
            variant="tonal"
...
onBeforeMount(() => {
  uData.value = store.getUsers();
  qData.value = store.getQuizData();
  store.isAdmin = store.getIsAdmin();
  store.loggedInUser = store.getLoggedInUser();
});
</script>
